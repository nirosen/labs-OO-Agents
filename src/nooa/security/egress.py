...
class UnsupportedEffectEgressVersionError(ValueError):
    ...
    def __init__(self, line_number: int, schema_version: object) -> None:
        ...
        super().__init__(
            f"unsupported effect egress schema_version at line {line_number}: "
            f"{schema_version!r}"
        )
...
class EffectEgressFrameTooLargeError(ValueError):
    ...
    def __init__(self, max_frame_bytes: int, *, line_number: int | None = None) -> None:
        ...
        super().__init__(
            f"effect egress frame{location} exceeds max_frame_bytes={max_frame_bytes}"
        )
    ...
    *,
    ...
    while True:
        ...
        if not isinstance(line, bytes):
            raise TypeError(
                f"read_effect_egress expected binary stream, got {type(line).__name__}"
            )
        if not line:
            break
...
def _validate_max_frame_bytes(max_frame_bytes: int) -> int:
    ...
    if not isinstance(max_frame_bytes, int) or isinstance(max_frame_bytes, bool):
        raise TypeError(
            f"max_frame_bytes expected int, got {type(max_frame_bytes).__name__}"
        )
