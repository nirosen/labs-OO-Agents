...
| V11 addition | The example supervisor now publishes one structured `finding_admission_refusal` stage after clearing any child-supplied value. The new matrix column splits the single `() / () / () / False / 0` shape into four stage buckets: count drift, scope drift, duplicate IDs, and pre-admission refusals. The two scope faults still share `scope_drift`, and malformed detector reports plus stale receipt scope still share `None`. |
