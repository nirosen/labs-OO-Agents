...
## Security Review Follow-On: Refusal Text Redaction

> Branch: `codex/security-refusal-text-redaction`

This slice adds no public `nooa.security` API. The example harness now stops interpolating core error strings for six row-validation refusal paths: the three detector-side receipt gates and the three supervisor-side finding gates that already have structured stage diagnostics from V16 and V11. Each harness-built refusal keeps its stage prefix, canonical reasons, caller-supplied expectation when one exists, and a `redacted_id_count`, while omitting the inspected receipt or finding row identifiers themselves.

```mermaid
flowchart LR
    R["receipt row gates<br/>ID / source / scope"] --> F["_redacted_row_refusal_detail()"]
    G["finding row gates<br/>scope / ID / required ref"] --> F
    F --> O["refusal_reason<br/>stage + reasons + redacted_id_count"]
    E["core error __str__<br/>still includes row identifiers"] -. "unchanged" .-> F
    S["subprocess stderr / arbitrary detector text"] -. "out of scope" .-> O
```

| Review item | Detail |
| --- | --- |
| Adds | Example-local `_redacted_row_refusal_detail()`, six row-validation refusal summaries, negative regression tests for omitted row IDs, and docs that pin the remaining non-redacted surfaces |
| Security claim | The example no longer embeds inspected row identifiers in the refusal text it constructs for the six current row-validation gates that already expose structured stage diagnostics; the stage prefix, canonical reasons, caller-selected expectations, and a redacted-identifier count remain. |
| Non-claim | This is not identifier protection or general sanitization. Core error messages still contain identifiers for callers that log them directly, subprocess stderr and tracebacks are untouched, evidence-membership refusal text remains unredacted because it has no structured stage field yet, detector-side refusal text is redacted by the detector and not verified by the supervisor, arbitrary detector-authored text is preserved, and caller-supplied expectations such as `expected_run_id` are deliberately retained for diagnosability. |
| Base slice | `codex/security-hardening-e2e-detector-pipeline-v16` |
...
| Validation | `pytest tests/security/test_detector_harness.py tests/security/test_surface_guide.py`; `pytest tests/security`; `ruff check examples/security_hardening tests/security`; `pyright examples/security_hardening/detector_harness.py` |
...
| Non-claim | Presence is not provenance. This does not authenticate findings, reports, scorers, evidence IDs, or the required ref; prove that the cited ref resolves to anything real or supports a finding; require any other evidence; prove detector coverage; or stop a detector that simply echoes the supervisor-supplied `input_id`. The current harness keeps the caller-supplied required ref for diagnosability but no longer includes inspected finding row IDs in this refusal text; core error strings remain unchanged. |
...
| Non-claim | This does not authenticate the detector, report bytes, finding producer, or `expected_run_id`; prove detector coverage; enforce finding-id uniqueness; dereference evidence refs; assign severity or verdict; or cap memory before `communicate()` collects stdout. The current harness keeps caller-supplied scope expectations but no longer includes inspected finding row IDs in its finding-scope refusal text; core error strings remain unchanged. |
...
| Non-claim | This is not automatic `DetectorInput` enforcement, receipt authentication, source authentication, coverage proof, sandboxing, or attestation. The current harness keeps caller-supplied scope expectations but no longer includes inspected receipt row IDs in its receipt-scope refusal text; core error strings and arbitrary detector-authored text remain unchanged. |
...
## Security Review Joint Branch: End-to-End Detector Pipeline V17
...
flowchart LR
    ...
    RI --> RSA["receipt_source_alignment_gated_scorer()<br/>identity only"]
    RSA --> G["receipt_scope_gated_scorer()<br/>identity only"]
    ...
    RI -. "duplicate_receipt_id" .-> RAD["receipt_admission_refusal<br/>detector-supplied"]
    RSA -. "receipt_source_mismatch" .-> RAD
    G -. "scope_drift" .-> RAD
    RAD --> X
    RI -. "row IDs redacted" .-> RT["refusal_reason<br/>reasons + redacted_id_count"]
    RSA -. "row IDs redacted" .-> RT
    G -. "row IDs redacted" .-> RT
    ...
    FAD --> X
    FG -. "row IDs redacted" .-> RT
    FU -. "row IDs redacted" .-> RT
    FR -. "row IDs redacted" .-> RT
    RT -. "six structured row gates only" .-> X
    X --> F
    VA -. "over-bound / invalid / scenario drift" .-> Y["SupervisorAdmissionError"]
    AA -. "over-bound / invalid" .-> Y
```

The matrix covers paths that return `DetectedScenario`. Victim and authority summary admission faults raise `SupervisorAdmissionError` instead, so their exact refusal shapes stay in `tests/security/test_detector_harness.py` rather than being flattened into matrix rows. The matrix still has no row for detector-side evidence-ref membership because current scorers emit refs within their `DetectorInput`, the current fault axis mutates outputs only after scoring, and the supervisor still has no independent evidence-ID set to recheck membership after bundle admission. V13 adds one row for the supervisor-side required-ref gate because `drop_required_evidence_ref` can strip the anchor after scoring and before finding admission. V14 adds one detector-side row for `duplicate_receipt_id`: the authority can duplicate a transported receipt after issuance while keeping the receipt bundle count coherent, so the receipt-ID wrapper can refuse before receipt scope or policy runs. V15 adds one detector-side row for `mislabel_receipt_source`: the authority can rewrite one transported row source while keeping the receipt bundle count coherent, so the source-alignment wrapper can refuse after receipt-ID uniqueness and before receipt scope or policy runs. V16 adds no row; it adds one detector-supplied `Receipt admission refusal` column that preserves the three post-completeness receipt gate stages without parsing `refusal_reason`. The prior four-row collision splits completely: malformed report remains `None`, stale receipt scope is `scope_drift`, duplicate receipt ID is `duplicate_receipt_id`, and mislabeled receipt source is `receipt_source_mismatch`. The remaining structurally identical pair is blank and stale finding scope, which still share `scope_drift` in the supervisor-owned `Finding admission refusal` column. V17 adds no row or column; it changes only the harness-built `refusal_reason` text for the six row-validation gates that already expose structured receipt or finding refusal stages. The matrix therefore stays at 22 rows and 13 columns because `refusal_reason` is not a matrix field, while evidence-membership refusal text remains outside this redaction because it still has no structured stage field.

<!-- JOINT_SCENARIO_MATRIX_START -->
| Scenario | Victim fault | Authority fault | Detector fault | Subprocess output fault | Profile | V2 effect signals | Receipt bundle signals | Finding bundle signals | Receipt admission refusal | Finding admission refusal | Scored | Findings |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `vulnerable_attack` | `none` | `none` | `none` | `none` | `identity_approval` | `()` | `()` | `()` | `None` | `None` | `True` | `1` |
| `vulnerable_attack` | `none` | `none` | `none` | `malformed_detector_report` | `identity_approval` | `()` | `()` | `()` | `None` | `None` | `False` | `0` |
| `vulnerable_attack` | `none` | `none` | `blank_finding_run_id` | `none` | `identity_approval` | `()` | `()` | `()` | `None` | `"scope_drift"` | `False` | `0` |
| `vulnerable_attack` | `none` | `none` | `stale_finding_run_id` | `none` | `identity_approval` | `()` | `()` | `()` | `None` | `"scope_drift"` | `False` | `0` |
| `vulnerable_attack` | `none` | `none` | `duplicate_finding_id` | `none` | `identity_approval` | `()` | `()` | `()` | `None` | `"duplicate_finding_id"` | `False` | `0` |
| `vulnerable_attack` | `none` | `none` | `drop_required_evidence_ref` | `none` | `identity_approval` | `()` | `()` | `()` | `None` | `"missing_required_evidence_ref"` | `False` | `0` |
| `vulnerable_attack` | `none` | `none` | `truncate_finding_document` | `none` | `identity_approval` | `()` | `()` | `("truncated",)` | `None` | `None` | `False` | `0` |
| `vulnerable_attack` | `none` | `none` | `drop_finding_count_mismatch` | `none` | `identity_approval` | `()` | `()` | `()` | `None` | `"count_mismatch"` | `False` | `0` |
| `defender_only_attack` | `none` | `none` | `none` | `none` | `identity_approval` | `()` | `()` | `()` | `None` | `None` | `True` | `0` |
| `hardened_authorized` | `none` | `none` | `none` | `none` | `identity_approval` | `()` | `()` | `()` | `None` | `None` | `True` | `0` |
| `hardened_authorized` | `none` | `stale_receipt_run_id` | `none` | `none` | `identity_approval` | `()` | `()` | `()` | `"scope_drift"` | `None` | `False` | `0` |
| `hardened_authorized` | `none` | `duplicate_receipt_id` | `none` | `none` | `identity_approval` | `()` | `()` | `()` | `"duplicate_receipt_id"` | `None` | `False` | `0` |
| `hardened_authorized` | `none` | `mislabel_receipt_source` | `none` | `none` | `identity_approval` | `()` | `()` | `()` | `"receipt_source_mismatch"` | `None` | `False` | `0` |
| `hardened_authorized` | `none` | `truncate_receipt_document` | `none` | `none` | `identity_approval` | `()` | `("truncated",)` | `()` | `None` | `None` | `False` | `0` |
| `hardened_authorized` | `none` | `drop_receipt_count_mismatch` | `none` | `none` | `identity_approval` | `()` | `("receipt_count_mismatch",)` | `()` | `None` | `None` | `False` | `0` |
| `vulnerable_attack` | `partial_tail_crash` | `truncate_receipt_document` | `none` | `none` | `identity_approval` | `("truncated", "missing_stream_end")` | `("truncated",)` | `()` | `None` | `None` | `False` | `0` |
| `export_vulnerable_attack` | `none` | `none` | `none` | `none` | `data_export` | `()` | `()` | `()` | `None` | `None` | `True` | `1` |
| `export_hardened_attack` | `none` | `none` | `none` | `none` | `data_export` | `()` | `()` | `()` | `None` | `None` | `True` | `0` |
| `vulnerable_attack` | `partial_tail_crash` | `none` | `none` | `none` | `identity_approval` | `("truncated", "missing_stream_end")` | `()` | `()` | `None` | `None` | `False` | `0` |
| `vulnerable_attack` | `exit_between_frames` | `none` | `none` | `none` | `identity_approval` | `("missing_stream_end",)` | `()` | `()` | `None` | `None` | `False` | `0` |
| `vulnerable_attack` | `drop_record_count_mismatch` | `none` | `none` | `none` | `identity_approval` | `("record_count_mismatch",)` | `()` | `()` | `None` | `None` | `False` | `0` |
| `export_vulnerable_attack` | `sequence_gap` | `none` | `none` | `none` | `data_export` | `("first_sequence_error",)` | `()` | `()` | `None` | `None` | `False` | `0` |
<!-- JOINT_SCENARIO_MATRIX_END -->

| Review item | Detail |
| --- | --- |
| Included slices | `codex/security-hardening-e2e-detector-pipeline`, `codex/security-second-victim-detector-generality`, `codex/security-effect-egress-stream-end-v2`, `codex/security-surface-guide`, `codex/security-lossy-writer-fault-coverage`, `codex/security-effect-egress-producer-conformance`, `codex/security-transport-conformance-vectors`, `codex/security-minimal-out-of-process-recipe`, `codex/security-receipt-scope-validation`, `codex/security-detector-receipt-scope-gate`, `codex/security-receipt-bundle-transport`, `codex/security-receipt-bundle-conformance-vectors`, `codex/security-receipt-bundle-version-contract`, `codex/security-finding-scope-gate`, `codex/security-supervisor-output-admission`, `codex/security-finding-bundle-handoff`, `codex/security-finding-bundle-conformance-vectors`, `codex/security-finding-id-uniqueness`, `codex/security-finding-admission-diagnostics`, `codex/security-finding-evidence-ref-membership`, `codex/security-finding-required-evidence-ref`, `codex/security-receipt-id-uniqueness`, `codex/security-receipt-source-alignment`, `codex/security-receipt-admission-diagnostics`, `codex/security-refusal-text-redaction` |
| V17 addition | The example harness now omits inspected receipt and finding row identifiers from the refusal text it constructs for the six current row-validation gates that already expose structured receipt or finding refusal stages. The matrix stays at 22 rows and 13 columns because `refusal_reason` is not a matrix field. |
| Security claim | Applications can compose and review one deterministic hardening path around current NOOA primitives: observe effects, keep authorization policy application-owned, move detector scoring outside the victim process, preserve V2 reader-visible refusal semantics, refuse reader-visible receipt-bundle truncation or count mismatch before receipt scope and identity policy run, refuse repeated `receipt_id` values in one supplied identity receipt iterable before receipt source alignment and identity policy run, refuse visible receipt source-label drift after receipt-ID uniqueness and before receipt scope or identity policy runs, refuse visible stale receipt scope before identity policy runs, expose those three current detector-side receipt-admission refusal stages through one example-local structured field instead of parsing refusal text, refuse detector-side scorer outputs that cite refs outside the current `DetectorInput` ID set before finding bundle emission, admit every parsed child stdout payload through bounded shape checks, refuse visible detector-report run or input-ID drift, refuse reader-visible finding-bundle truncation or report-to-bundle count drift before finding scope runs, refuse visible finding scope drift, refuse repeated `finding_id` values in one admitted finding iterable after existing scope and coherence gates pass, refuse admitted rows that omit the supervisor-selected `input_id` after those gates pass, identify current finding-admission refusal stages through an example-local structured field instead of parsing refusal text, omit inspected row identifiers from the harness-built refusal text for those six structured row-validation gates while keeping stage prefixes, canonical reasons, caller-selected expectations, and redacted-identifier counts, reject victim-summary scenario drift before scenario consumers accept it, and reuse the same detector handoff across two example victim policies. |
| Non-claim | This remains a same-user, same-host example with no sandboxing, signing, authentication, attestation, production IAM boundary, or completeness proof. Two victims do not establish general coverage. A valid-looking V2 terminator, receipt bundle, or finding bundle can still be forged; matching future bundle version tokens are not promises of parser support or safety; a dishonest writer can omit an effect, receipt, or finding and declare the reduced count; receipts and findings remain caller-controlled copies; count agreement, scope consistency, distinct receipt IDs, receipt source-label agreement, distinct finding IDs, evidence-ref membership against a detector input, required-ref presence, `receipt_admission_refusal`, and `finding_admission_refusal` are not authenticity, provenance, evidence support, or cross-bundle uniqueness; the detector sees the supervisor-selected `input_id` and can echo it mechanically; `receipt_admission_refusal` is detector-supplied and unverified, so a detector can report any stage or `None` exactly as it can with `refusal_reason`; the receipt-ID, receipt-source, and evidence-ref wrappers are not supervisor gates and a hostile detector can bypass them; both structured refusal fields are example-local rather than public NOOA schemas; clean empty bundles are not evidence that nothing exists; a well-formed victim or authority summary can still lie; the three stdout byte bounds apply only after `communicate()` has collected stdout; the finding-bundle bound applies only when the supervisor reads its same-user temporary file; core error strings, subprocess stderr, report-admission text, evidence-membership refusal text, and arbitrary detector-authored text can still expose raw identifiers; and zero findings remain policy-specific example outcomes rather than a general safety verdict. |
